...
class Mem2Seq(nn.Module):
	def __init__(self, hidden_size, max_len, max_p, lang, path, lr, n_layers, dropout, relation_size, batch_size):
		...
		self.decoder = Decoder(max_p, max_len, hidden_size, dropout, lang.n_words, embedding_dim = hidden_size, relation_size = relation_size, batch_size = batch_size)
		self.decoder_optimizer = optim.Adam(self.decoder.parameters(), lr=lr)
...
#        print(decoder_hidden.size())
		...
		h = decoder_hidden
		c = Variable(torch.zeros(self.batch_size, 1, self.hidden_size))
		if USE_CUDA:
			c = c.cuda()
		...
		print(len(target_batches), len(target_batches[0]))
#        heads = [a[0] for a in target_batches]
#        tails = [a[1] for a in target_batches]
#        relas = Variable(torch.from_numpy(np.array([a[2] for a in target_batches])).long())
#        loss, r = self.decoder.extract_ralation(heads, tails, relas)
...
#        return u_k
	def forward(self, story):
		...
		for hop in range(self.max_hops):
#            print(story.size())
			embed_A = self.C[hop](story.contiguous().view(b, -1).long())  # b * (m * s) * e
			u_temp = u[-1].unsqueeze(1).expand_as(embed_A)
			prob = self.softmax(torch.sum(embed_A * u_temp, 2))
			embed_C = self.C[hop + 1](story.contiguous().view(b, -1).long())
			prob = prob.unsqueeze(2).expand_as(embed_C)
			o_k = torch.sum(embed_C * prob, 1)
			u_k = u[-1] + o_k
			u.append(u_k)
...
class Decoder(nn.Module):
	def __init__(self, max_p, max_len, hidden_size, dropout, vocab, embedding_dim, relation_size, batch_size):
		...
		self.A = [Variable(torch.zeros(1, hidden_size)) for i in range(batch_size)]

		self.output_pointer = 0
		self.outputs = [Variable(torch.zeros(max_len, hidden_size)) for i in range(batch_size)]
		if USE_CUDA:
	...
	def forward(self, sent, h, c):
		...
		for i in range(sent.size(0)):
			att = self.softmax(F.linear(sent[i], self.A[i]))
			att = F.linear(att, self.A[i].transpose(0, 1)  
			sent[i] = sent[i] + att
...
#            print(self.outputs[self.output_pointer][i].size(), o[0][0].size())
			for j in range(sent.size()[0]):
				self.outputs[j][self.output_pointer] = o[0][0]
			self.output_pointer += 1
		
#        print(self.A.size(), h.size())
		for i in range(sent.size()[0]):
			self.A[i] = torch.cat((self.A[i], h[i]), dim = 0)
#        self.pointer += 1
